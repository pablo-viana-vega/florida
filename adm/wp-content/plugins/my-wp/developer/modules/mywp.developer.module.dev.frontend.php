...
*/
    ...
    if( is_singular() ) {
      ...
      if( ! empty( $post ) && is_object( $post ) ) {
        ...
        $debug_lists['get_post( ' . $post->ID . ' )'] = get_post( $post->ID );
        $debug_lists['MywpPostType::get_post( ' . $post->ID . ' )'] = MywpPostType::get_post( $post->ID );
        $debug_lists['SELECT * FROM $wpdb->posts WHERE ID = ' . $post->ID] = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE ID = %d", $post->ID ) );
