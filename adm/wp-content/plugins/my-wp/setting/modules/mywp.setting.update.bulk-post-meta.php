...
final class MywpSettingScreenUpdateBulkPostMeta extends MywpAbstractSettingModule {
  ...
  public static function ajax_show_bulk_post_rows() {
    ...
    if( ! empty( $_POST['bulk_meta_value'] ) ) {
  ...
  public static function mywp_current_admin_print_footer_scripts() {
    ...
    jQuery(function( $ ) {
      ...
      var $setting_update_bulk_post_meta_posts = $('#setting-update-bulk-post-meta-posts');

      var $setting_update_bulk_post_meta_update_form = $('#setting-update-bulk-post-meta-update-form');

      var $setting_update_bulk_post_meta_update_results = $('#setting-update-bulk-post-meta-update-results');
      ...
      $('#setting-update-bulk-post-meta-show-posts').on('click', function() {
        ...
        var $setting_update_bulk_post_meta_filters = $(this).parent();

        var selected_post_type = $setting_update_bulk_post_meta_filters.find('#setting-update-bulk-post-meta-selected-post-type').val();

        var selected_post_status = $setting_update_bulk_post_meta_filters.find('#setting-update-bulk-post-meta-selected-post-status').val();
      ...
      $(document).on('click', '#update-bulk-post-meta-all-posts-check', function() {
        ...
        $table.find('tbody tr').each( function( index , el ) {
          ...
          var $check = $(el).find('th.check .update-bulk-post-meta-select-post-id');
      ...
      function get_selected_posts() {
        ...
        var $list_table = $(document).find('#setting-update-bulk-post-meta-posts .wp-list-table');
      ...
      function change_meta_update_form() {
        ...
        var selected_posts = get_selected_posts();

        var $meta_update_form_selected_posts = $setting_update_bulk_post_meta_update_form.find('.active-content table tr.selected-posts td');
      ...
      $('.setting-update-bulk-post-meta-update-post-meta').on('click', function() {
        ...
        var is_do_run = parseInt( $(this).data('do_run') );

        var $update_form = $(this).parent().parent().parent();

        var $spinner = $update_form.find('.spinner');

        var meta_key = $update_form.find('#setting-update-bulk-post-meta-update-post-meta-key').val();

        var bulk_meta_value = $update_form.find('#setting-update-bulk-post-meta-update-post-meta-val').val();
        ...
        if( is_do_run === 1 ) {
          ...
          var is_confirm = window.confirm('<?php _e( 'Are you sure you want to update?' , 'my-wp' ); ?>');
        ...
        $.ajax({
          ...
          var scroll_position = $setting_update_bulk_post_meta_update_results.offset().top;
      ...
      function update_post_meta( is_do_run , meta_key , bulk_meta_value ) {
        ...
        var $setting_update_bulk_post_meta_update_results = $(document).find('#setting-update-bulk-post-meta-update-results');

        var $update_post = $setting_update_bulk_post_meta_update_results.find('.wp-list-table tbody tr.result-post.wait').first();

        var $is_updated = $update_post.find('td.is-updated');
  ...
  public static function mywp_current_setting_screen_content() {
    ...
    <div id="setting-update-bulk-post-meta-update-form">
      ...
      <h3 class="mywp-setting-screen-subtitle">3: <?php _e( 'Bulk update post meta' , 'my-wp' ); ?></h3>
  ...
  private static function print_post_items( $posts , $post_args ) {
    ...
    <p><code><?php print_r( $post_args ); ?></code></p>
