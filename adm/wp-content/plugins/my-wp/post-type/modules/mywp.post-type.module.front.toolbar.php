...
if ( ! class_exists( 'MywpPostTypeModuleFrontToolbar' ) ) :

final class MywpPostTypeModuleFrontToolbar extends MywpPostTypeAbstractModule {

  protected static $id = 'mywp_front_toolbar';

  protected static function get_regist_post_type_args() {

    $args = array(
      'label' => 'My WP Frontend Toolbar',
      'hierarchical' => true,
  ...
  public static function current_manage_posts_custom_column( $column_name , $post_id ) {
    ...
    if( $column_name === 'order' ) {
      ...
      if( $mywp_post->item_link_title ) {
        ...
        if( strpos( $mywp_post->item_link_title , '<input ' ) !== false ) {

          $mywp_post->item_link_title = esc_html( $mywp_post->item_link_title );
...
MywpPostTypeModuleFrontToolbar::init();
