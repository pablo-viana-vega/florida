...
class ReadMoreData {
	...
	public static function getAllDataByLimit($offset, $limit = 1,$orderBySql = 'Order by id',$orderSql = 'desc') {
		
		global $wpdb;
		
		$results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}expm_maker $orderBySql $orderSql LIMIT $offset, $limit", ARRAY_A);
		
		return $results;
	}

	public static function getAllSearchSaved($offset, $limit = 1,$orderBySql = 'Order by id',$orderSql = 'desc') {

		global $wpdb;

		$results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}".YRM_FIND_TABLE." $orderBySql $orderSql LIMIT $offset, $limit", ARRAY_A);
